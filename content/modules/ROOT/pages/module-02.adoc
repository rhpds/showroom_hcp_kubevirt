...
Login to {openshift_cluster_console_url}/dashboards[window=_blank] with the user `{openshift_cluster_admin_username}` and the password `{openshift_cluster_admin_password}`
...
It is possible to use the UI to create a cluster on *OpenShift Virtualization* but this lab focus on using the command line.
