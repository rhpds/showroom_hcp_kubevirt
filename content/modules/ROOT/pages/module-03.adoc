...
. The list of the pods created are the following:
** capi-provider
** catalog-operator
...
** ignition-server  (x3))
** ignition-server-proxy  (x3))
** ingress-operator
** konnectivity-agent  (x3))
** kube-apiserver  (x3))
** kube-controller-manager  (x3))
** kube-scheduler  (x3))
** kubevirt-cloud-controller-manager
** kubevirt-csi-controller
** machine-approver
** multus-admission-controller
** network-node-identity (x3)
** oauth-openshift (x3)
** olm-operator
** openshift-apiserver (x3))
** openshift-controller-manager (x3))
** openshift-oauth-apiserver (x3))
** openshift-route-controller-manager (x3))
** ovnkube-control-plane (x3))
** packageserver (x3))
...
. Navigate to the `Console URL` and accept the self-signed certificate
