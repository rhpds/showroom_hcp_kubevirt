...
. Finally add your cluster `cluster1` to the new `ManagedClusterSet`:
+
[source,sh,role=execute]
----
oc label managedcluster cluster1 cluster.open-cluster-management.io/clusterset=development --overwrite
----

. If you check back in the OpenShift Web Console you should now see that the `ManagedClusterSet` *development* has been created and that your hosted cluster *cluster1* is now a part of the `ManagedClusterSet`.
...
[source,sh,role=execute]
----
oc -n clusters patch hostedcluster cluster1 --type=merge --patch='{"spec":{"configuration":{"oauth":{"identityProviders":[{"name":"development","type":"HTPasswd","htpasswd":{"fileData":{"name": "htpasswd-development"}},"mappingMethod":"claim"}],"templates":{"error":{"name":""},"login":{"name":""},"providerSelection":{"name":""}},"tokenConfig":{}}}}}'
----
...
[source,sh,role=execute]
----
oc get managedcluster cluster1 -o json | jq -r '.status.clusterClaims[] | select(.name == "consoleurl.cluster.open-cluster-management.io") | .value'
----
+
.Sample Output
[source,text,options=nowrap]
----
https://console-openshift-console.apps.cluster1.apps.d965w.dynamic.redhatworkshops.io
----
...
[source,text,options=nowrap]
----
cluster1               development-policies.openshift-pipelines-installed   enforce              Compliant          64s
development-policies   openshift-pipelines-installed                        enforce              Compliant          3m12s
----
+
Note that the policy in the `development-policies` shows as *Compliant* - and that the policy has been copied to the one cluster in your `ManagedClusterSet` - *cluster1*.
...
[source,text,options=nowrap]
----
cluster1               development-policies.openshift-gitops-installed      enforce              Compliant          13s
...
[source,text,options=nowrap]
----
cluster1               development-policies.openshift-serverless-installed   enforce              NonCompliant       24s
